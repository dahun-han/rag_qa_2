"""checkpoint_io.py — 체크포인트 저장(retrieval/answer json append, 재시도, 이전 결과 정리).
...
from export import build_answer_records, build_retrieval_records, save_json_items
...
def save_checkpoint(
    chunk_pool_by_method: dict, cfg, new_items_by_method: dict, sims_by_method: dict, output_dir: str,
) -> None:
    """이번 체크포인트에서 새로 추가된 항목(new_items_by_method, 청킹방식별)만 retrieval/
    answer json으로 이어서 저장한다 - 항목(index)당 파일 1개라, 이전 체크포인트에서
    이미 쓴 내용은 다시 읽거나 재작성하지 않는다. 이 폴더는 OneDrive 동기화 대상이라
    (_save_with_retry의 PermissionError 재시도가 그 증거), 안 바뀐 내용을 매번 다시
    쓰면 OneDrive 재업로드로 잠금 경합이 커진다 — 새 항목만 쓰면 이 문제가 없다."""
    for method in chunk_pool_by_method:
        ...
        _save_with_retry(
            save_json_items, build_retrieval_records(new_items), f"{output_dir}/retrieval",
        )
        _save_with_retry(
...
def _clear_output_files(output_dir: str) -> None:
    """완성 순서 기준 번호로 이미 저장된 retrieval/answer json을 지운다 - 정렬된 새
    seq로 다시 저장하기 위한 선행 작업(재번호 매김 전 파일이 새 파일과 뒤섞이면 번호가
    겹치거나 옛 파일이 잔류할 수 있다)."""
